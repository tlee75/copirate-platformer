...
func display_actions_for_item(stack, context = {}):
	...
	craft_button.pressed.connect(func(): CraftingManager.craft_item(stack.item))
	$ButtonContainer.add_child(craft_button)


func display_craft_action(stack: InventoryManager.ItemStack):
	var craft_button = Button.new()
	craft_button.text = "Craft " + stack.item.name
	craft_button.pressed.connect(func(): action_requested.emit(InventoryActionResolver.ActionType.CRAFT, stack))
	$ButtonContainer.add_child(craft_button)
